...
var unigSkills = _.uniq(_.flatten((_.map(persons, 'skills'))));
var sortedSkills = _.sortBy((unigSkills), _.toLower);

console.log('Массив скиллов (поле skills) всех людей без повторов',sortedSkills);

//2. Массив имен (поле name) людей, отсортированных в зависимости от количества их друзей (friends)

var sortedPersons = _.orderBy(persons, ['friends'], ['asc']);
var sortedPersonsName = _.map(sortedPersons, 'name');

console.log('Массив имен (поле name) людей, отсортированных в зависимости от количества их друзей (friends) по возрастанию',sortedPersonsName );

//3. Массив всех друзей всех пользователей, не должно быть повторяющихся людей

var allFriends= _.flatten(_.map(persons, 'friends'));

console.log('Массив всех друзей всех пользователей',  _.sortBy(_.uniq(_.map(allFriends, 'name'))));
